...
const Events = () => {
    ...
    const [link, setLink] = React.useState('/api/eventslist');
    ...
    const handleClickchange = (e) => {
        ...
        if (pagenomber == 1) {
            setLink('/api/eventslist/');
        }
        else {
            setLink(`/api/eventslist/?page=${pagenomber}`);
        }
    }
    React.useEffect(() => {
        getBusiness();
                    ...
                    <div className="col-lg-8 col-xl-9 position-relative overflow-hidden pb-5 pt-4 px-3 px-xl-4 px-xxl-5">
                        ...
                        <nav className="mb-3 pt-md-2" aria-label="Breadcrumb">
                            <ol className="breadcrumb">
                                <li className="breadcrumb-item"><Link href="city-guide-home-v1.html" to="/catalog">Home</Link></li>
                                <li className="breadcrumb-item active" aria-current="page">Find Events</li>
                            </ol>
                        </nav>
                        {/* Title*/}
                        <div className="d-sm-flex align-items-center justify-content-between pb-3 pb-sm-4">
                            <h1 className="h2 mb-sm-0">Find Events</h1><Link className="d-inline-block fw-bold text-decoration-none py-1" to="/events" data-bs-toggle-className="invisible" data-bs-target="#map"><i className="fi-map me-2" />Map view</Link>
                        </div>
                        {/* Sorting*/}
