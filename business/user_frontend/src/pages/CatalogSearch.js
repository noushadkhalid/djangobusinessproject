...
const CatalogSearch = () => {
    ...
    const [link, setLink] = React.useState(`/api/search_view/?item=${title}&location=${location}`);
    ...
    const handleClickchange = (e) => {
        ...
        if (pagenomber == 1) {
            setLink(`/api/search_view/?item=${title}`);
        }
        else {
            setLink(`/api/search_view/?item=${title}&page=${pagenomber}`);
        }
    }
    React.useEffect(() => {
        getBusiness();
                ...
                <div className="row g-0 mt-n3">
                    ...
                    <div className="col-lg-8 col-xl-9 position-relative overflow-hidden pb-5 pt-4 px-3 px-xl-4 px-xxl-5">
                        ...
                        <div className="d-sm-flex align-items-center justify-content-between pb-3 pb-sm-4">
                            <h1 className="h2 mb-sm-0">Find Results</h1><a className="d-inline-block fw-bold text-decoration-none py-1" href="#" data-bs-toggle-className="invisible" data-bs-target="#map"><i className="fi-map me-2" />Map view</a>
                        </div>
                        {/* Sorting*/}
