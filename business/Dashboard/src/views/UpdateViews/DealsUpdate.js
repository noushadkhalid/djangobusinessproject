...
const DealsUpdate = () => {
    ...
    const updatebusines = async (e) => {
        ...
        if (e.target.open_at) {
            formdata.append("timingfrom", e.target.open_at.value);
        }
        if (e.target.close_at) {
            formdata.append("timingto", e.target.close_at.value);
        }
        if (e.target.phone_number) {
        ...
        if (e.target.active) {
            formdata.append("active", e.target.active.value);
        }
        const csrf_token = getCookie('csrftoken');
        const url = `/api/updatedeal/${slug}/`;
                                    ...
                                    <div className="col-md-12">
                                        <div className="card card-primary">
                                            <div className="card-header">
                                                <h3 className="card-title">Update Deals</h3>
                                            </div>
                                            <form onSubmit={updatebusines}>
                                                <div className="card-body">
                                                    ...
                                                    <div className="form-group">
                                                        ...
                                                        <input type="tel" className="form-control" id="exampleInputDescription" placeholder="0332-3272-719" name="phone_number" pattern="[0-9]{4}-[0-9]{4}-[0-9]{3}" />
                                                        <small>Format: 0332-3272-719</small>
                                                    </div>
                                                    <div className="form-group">
                                                        <label htmlFor="exampleInputPassword1">Activated From: Example(06/24/2022 12:55 AM)</label>
                                                        <input type="text" className="form-control" id="exampleInputLocation" placeholder="Enter Time" name="open_at"  />
                                                    </div>
                                                    <div className="form-group">
                                                        <label htmlFor="exampleInputPassword1">Activated Till: Example(06/24/2022 12:55 AM)</label>
                                                        <input type="text" className="form-control" id="exampleInputLocation" placeholder="Enter Time" name="close_at" />
                                                    </div>
                                                    <div className="form-group">
                                                        <div className="custom-control custom-checkbox">
                                                            <input className="custom-control-input" type="checkbox" id="customCheckbox1" checked={checked} value={checked ? true : false} name="active" onChange={handleValue} />
                                                            <label htmlFor="customCheckbox1" className="custom-control-label">Active</label>
                                                        </div>
                                                    </div>
                                                    <div className="form-group">
